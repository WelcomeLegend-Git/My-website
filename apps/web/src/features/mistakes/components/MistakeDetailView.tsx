    ...
    <div className="w-full">
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-3">
                <div className="p-3 rounded-xl bg-gradient-to-br from-red-500 to-pink-500 shadow-lg shadow-red-500/25">
                  <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                </div>
                <div>
                  <h1 className="text-3xl font-bold text-slate-100">{mistake.title}</h1>
                  <div className="flex items-center gap-3 mt-2">
                    <span className={`px-2 py-1 rounded-md text-xs font-medium border ${getDifficultyColor(mistake.difficulty)}`}>
                      {mistake.difficulty}
                    </span>
                    <span className={`px-2 py-1 rounded-md text-xs font-medium border ${getErrorTypeColor(mistake.errorType)}`}>
                      {mistake.errorType}
                    </span>
                    <span className={`px-2 py-1 rounded-md text-xs font-medium border ${getStatusColor(mistake.status)}`}>
                      {mistake.status}
                    </span>
                  </div>
                </div>
            ...
            <div className="flex items-center gap-2">
              <button onClick={expandAll} className="px-4 py-2 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-sm font-medium hover:bg-red-500/20 transition-colors">
                Expand All
              </button>
              <button onClick={collapseAll} className="px-4 py-2 rounded-xl bg-slate-700/50 border border-slate-600 text-slate-300 text-sm font-medium hover:bg-slate-700 transition-colors">
                Collapse All
              </button>
            </div>
          </div>
        ...
        {imageAssets.length > 0 && (
          <div className="mb-8 rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900/80 to-slate-800/30 backdrop-blur p-6 shadow-xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {imageAssets.map((asset, index) => (
                <div key={asset.id} className="group relative rounded-xl overflow-hidden border border-slate-700 hover:border-blue-500 transition-all cursor-pointer"
                  onClick={() => onImageClick?.(asset.url, index, allImageUrls)}>
                  <img src={asset.url} alt={asset.caption || `Image ${index + 1}`} className="w-full h-48 object-cover" />
        ...
        <div className="mt-8 glass-card rounded-2xl p-6 border border-slate-800/50">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/quiz-history" className="px-4 py-2 rounded-xl bg-gradient-to-r from-primary/20 to-purple-500/20 border border-primary/30 text-primary font-medium hover:from-primary/30 hover:to-purple-500/30 transition-all flex items-center gap-2">
              View Quiz History
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
