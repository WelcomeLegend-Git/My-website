         ...
         <div className="space-y-3">
           {formulaItems.map((item: any) => (
             <button
               ...
               className="w-full text-left glass-card rounded-2xl border border-line hover:border-brass/30 bg-surface hover:bg-surface transition-all p-4 sm:p-5 flex items-start justify-between gap-3 hover-lift"
             >
               <div className="flex items-start gap-3 min-w-0">
                 <div className="w-10 h-10 rounded-xl bg-brass-soft flex items-center justify-center flex-shrink-0">
                 ...
                 <div className="min-w-0">
                   <p className="text-xs uppercase tracking-[0.25em] text-ink-muted mb-1">
                     {item.kind === "collection" ? "Collection" : "Formula"}
                   </p>
                   <p className="text-sm sm:text-base font-semibold text-ink truncate">{item.title}</p>
               ...
               {"formulaCount" in item && (
                 <span className="px-3 py-1 rounded-lg bg-brass-soft border border-line text-brass text-xs font-semibold flex-shrink-0">
         ...
         <div className="space-y-3">
           {mistakeItems.map((item: any) => (
             <button
               ...
               className="w-full text-left glass-card rounded-2xl border border-line hover:border-red-400/60 bg-surface hover:bg-surface transition-all p-4 sm:p-5 flex items-start justify-between gap-3 hover-lift"
             >
               <div className="flex items-start gap-3 min-w-0">
                 ...
                 <div className="min-w-0">
                   <p className="text-xs uppercase tracking-[0.25em] text-ink-muted mb-1">Mistake</p>
                   <p className="text-sm sm:text-base font-semibold text-ink truncate">{item.title}</p>
                   <p className="text-xs text-ink-muted mt-1 truncate">
                     {item.subjectName} • {item.chapterTitle}
                   </p>
         ...
         <div className="space-y-3">
           {quizItems.map((item: any) => (
             <button
               ...
               className="w-full text-left glass-card rounded-2xl border border-line hover:border-brass/40 bg-surface hover:bg-surface transition-all p-4 sm:p-5 flex items-start justify-between gap-3 hover-lift"
             >
               <div className="flex items-start gap-3 min-w-0">
                 <div className="w-10 h-10 rounded-xl bg-brass-soft flex items-center justify-center flex-shrink-0">
                   <svg className="w-5 h-5 text-brass" fill="none" stroke="currentColor" viewBox="0 0 24 24">
         ...
         {aiItems.map((item: any) => (
           ...
           >
             <div className="flex items-start gap-3 min-w-0">
               ...
               <div className="min-w-0">
                 <p className="text-xs uppercase tracking-[0.25em] text-ink-muted mb-1">Study Guru</p>
                 <p className="text-sm sm:text-base font-semibold text-ink truncate">{item.title}</p>
                 {item.metadata?.chapter && (
                   <p className="text-xs text-ink-muted mt-1 truncate">{item.metadata.chapter}</p>
                 )}
               </div>
             </div>
           </button>
     ...
     <section className="space-y-6 min-w-0">
       <header className="fade-in-up">
         <div className="flex items-center gap-3 mb-3">
           ...
           <div>
             <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-ink">Bookmarks</h1>
           ...
           return (
             <button
               ...
               className={`relative text-left rounded-2xl border p-4 sm:p-5 transition-all hover-lift ${
                 isActive
                   ? "border-brass/30 bg-brass-soft shadow-sm"
                   : "border-line bg-surface hover:border-brass/20 hover:bg-surface"
               }`}
             >
               <div
                 className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl ${card.iconBg} flex items-center justify-center mb-3`}
               >
                 <svg className="w-5 h-5 text-brass" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                   <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={card.iconPath} />
                 </svg>
               </div>
               <p className="text-sm sm:text-base font-semibold text-ink flex items-center gap-2">
                 {card.title}
                 {count > 0 && (
                   <span className="text-[11px] px-2 py-0.5 rounded-full bg-surface-2 text-ink-muted border border-line">
                     {count}
                   </span>
                 )}
               </p>
               <p className="mt-1 text-xs text-ink-muted">{card.subtitle}</p>
             </button>
           );
         })}
       </div>

       <div className="glass-card rounded-2xl border border-line p-4 sm:p-5 flex flex-col gap-3">
         <div className="flex flex-col lg:flex-row gap-3 lg:items-center">
           <div className="flex-1 min-w-0">
             <input
               ...
               className="w-full rounded-xl border border-line bg-paper px-3 py-2 text-sm text-ink placeholder-slate-500 focus:border-brass/30 focus:ring-2 focus:ring-brass/20 focus:outline-none"
             />
           </div>
         ...
         {isLoading ? (
           <div className="flex items-center justify-center py-12">
             <div className="text-center">
               <div className="w-12 h-12 border-4 border-brass/30 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
